...
- project_name: nombre del proyecto que ejecuta al job.
...
**FULL - DAY_PRECISION:** se toman los parámetros fecha_inicial y fecha_final de job_parameters para realizar la carga iterando desde la fecha inicio hasta la fecha fin.

**FULL - MONTH_PRECISION:** calcula el primer día del mes de fecha_inicial y el último día del mes de fecha_final de los job_parameters.

**INC - DAY_PRECISION:** calcula la fecha anterior a la ejecución (t-1) y se realiza la carga de esta fecha.

**INC - MONTH_PRECISION**: calcula el primer y último día del mes anterior a la fecha de ejecución y se realiza la carga de este rango.

Posteriormente valida el tipo de job, y crea el payload de acuerdo a el valor de entrada ‘raw', ‘staging’ o ‘raw-to-staging' como ya se explicó anteriormente.
